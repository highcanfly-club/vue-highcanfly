            ...
            <div class="flex-auto p-5 lg:p-10">
              <div v-if="!fromSendEmail">
                <form @submit="checkForm" @submit.prevent="submitForm">
                  ...
                  <h3 v-if="formerrors.length">
                    <b>Veuillez corriger ces erreurs:</b>
                    <ul>
                      <li v-for="error in formerrors" :key="formerrors.id"> <!-- eslint-disable-line -->
                        {{ error }}
                      </li>
                    </ul>
                  </h3>
                  <p class="leading-relaxed mt-1 mb-4 text-slate-500">
                    Completer ce formulaire…
                  </p>
...
export default {
  ...
  methods: {
    ...
    submitForm: function () {
      if (!this.formerrors.length && this.name.length && this.email.length && this.message.length ) {
