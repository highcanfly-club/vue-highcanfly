    ...
    <div class="px-6">
      <div class="flex flex-wrap justify-center">
        ...
        <div v-if="error" class="error">
         ...
         <div v-if="post" class="w-full px-4 text-center mt-20">
          <div class="flex justify-center py-4 lg:pt-4 pt-8">
            <div class="mr-4 p-3 text-center">
              <span
                class="text-xl font-semibold block tracking-wide text-slate-600"
              >
                {{ (new Date($data.post._createdAt)).toLocaleString(undefined,{weekday: "long", year: "numeric", month: "long", day: "numeric"}) }}
              </span>
              <span class="text-sm text-slate-400">Par {{ post.name }}</span>
            </div>
          </div>
        </div>       
      </div>
